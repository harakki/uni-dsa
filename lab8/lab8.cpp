...
// Приближенные алгоритмы A1 и A2
void create_optimal_search_tree_A1(Tree *&root)
{
    for (int i = 1; i <= TREE_SIZE; i++)
    {
        add_to_optimal_search_tree(V[i], W[i], root);
    }
}

void create_optimal_search_tree_A2(int Left, int Right, Tree *&root)
{
    int wes = 0, sum = 0, i;
    if (Left <= Right)
    {
        ...
        add_to_optimal_search_tree(V[i], W[i], root);
        create_optimal_search_tree_A2(Left, i - 1, root);
        create_optimal_search_tree_A2(i + 1, Right, root);
    }
}
...
int main()
{
    ...
    root_a1 = new Tree(V[1], W[1]);
    root_a2 = new Tree(V[1], W[1]);

    fill_matrix_weight();
    fill_matrix_AP_AR();

    create_optimal_search_tree_A1(root_a1);
    create_optimal_search_tree_A2(1, TREE_SIZE, root_a2);

    // A1
    cout << "ДОП-дерево (алгоритм A1)"
         << "\n\n"
         << "Обход дерева cлева направо:"
         << "\n";

    left_to_right(root_a1);

    cout << "\n"
         << "Размер дерева: " << tree_size(root_a1) << "\n"
         << "Контрольная сумма: " << tree_checksum(root_a1) << "\n"
         << "Средневзвешенная высота: " << tree_middle_height(root_a1) << "\n\n\n";

    // A2
    cout << "ДОП-дерево (алгоритм A2)"
         << "\n\n"
         << "Обход дерева cлева направо:"
         << "\n";

    left_to_right(root_a2);

    cout << "\n"
         << "Размер дерева: " << tree_size(root_a2) << "\n"
         << "Контрольная сумма: " << tree_checksum(root_a2) << "\n"
         << "Средневзвешенная высота: " << tree_middle_height(root_a2) << "\n";

#ifdef SFML_GRAPHICS_HPP

    sf::RenderWindow window_a1(sf::VideoMode(1900, 800), "A1");
    sf::RenderWindow window_a2(sf::VideoMode(1900, 800), "A2");

    while (window_a1.isOpen() || window_a2.isOpen())
    {
        sf::Event event;

        while (window_a1.pollEvent(event) || window_a2.pollEvent(event))
        {
        ...
        window_a1.clear(sf::Color::White);
        SFML_draw_tree(window_a1, root_a1, 1900);
        window_a1.display();

        window_a2.clear(sf::Color::White);
        SFML_draw_tree(window_a2, root_a2, 1900);
        window_a2.display();
    }
